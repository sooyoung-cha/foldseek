...
if notExists "${TMP_PATH}/multimer_clu.dbtype"; then
    # shellcheck disable=SC2086
    "$MMSEQS" multimercluster_fast "${QUERY}" "${TMP_PATH}/multimer_clu" "${TMP_PATH}/multimercluster_tmp" ${MULTIMERCLUSTERFAST_PAR} \
        || fail "multimercluster_fast died"
fi
...
if [ -n "${REMOVE_TMP}" ]; then
    ...
    rm -f -- "${TMP_PATH}/easymultimerclusterfast.sh"
fi
