...
#include "easymultimerclusterfast.sh.h"

void setEasyMultimerClusterFastDefaults(Parameters *p) {
    p->removeTmpFiles = true;
    p->writeLookup = true;
}

void setEasyMultimerClusterFastMustPassAlong(Parameters *p) {
    p->PARAM_REMOVE_TMP_FILES.wasSet = true;
    p->PARAM_WRITE_LOOKUP.wasSet = true;
}

int easymultimerclusterfast(int argc, const char **argv, const Command &command) {
    ...
    setEasyMultimerClusterFastDefaults(&par);
    par.parseParameters(argc, argv, command, true, Parameters::PARSE_VARIADIC, 0);
    setEasyMultimerClusterFastMustPassAlong(&par);
    ...
    cmd.addVariable("MULTIMERCLUSTERFAST_PAR", par.createParameterString(par.multimerclusterfastworkflow, true).c_str());
    ...
    std::string program = tmpDir + "/easymultimerclusterfast.sh";
    FileUtil::writeFile(program, easymultimerclusterfast_sh, easymultimerclusterfast_sh_len);
